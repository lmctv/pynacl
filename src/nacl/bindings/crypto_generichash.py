...
def generichash_blake2b_salt_personal(data,
    ...
    _slt = _pad_or_truncate(salt, crypto_generichash_SALTBYTES)
    _prs = _pad_or_truncate(personal, crypto_generichash_PERSONALBYTES)

    _salt = ffi.new("char []", crypto_generichash_SALTBYTES)
    _personal = ffi.new("char []", crypto_generichash_PERSONALBYTES)

    ffi.memmove(_salt, _slt, crypto_generichash_SALTBYTES)
    ffi.memmove(_personal, _prs, crypto_generichash_PERSONALBYTES)
